...
?>
<div id="ajaxLoader">
    <div align="center" ><img height="20px;" src="<?php echo public_path('../../images/ajax-loader.gif'); ?>"   />
</div>
</div>  
<div class="outerbox">
...
<?php }}} ?>
                ...
                <select name="cmbEtype" id="cmbEtype"  style="width: 120px;">
                        <option value=""><?php echo __("--Select--") ?></option>
                        <?php foreach ($EvaluationTypeList as $EvaluationType) {
                        ?>
                            <option value="<?php echo $EvaluationType->eval_type_id; ?>" <?php if ($searchEvaluationType == $EvaluationType->eval_type_id) { echo "selected"; } ?> style="width: 100px;"
                                    title="<?php if ($Culture == 'en') {
...
if($empNo==$Employee->sup_emp_number){
...
<!--                                        <input type='checkbox' class='checkbox innercheckbox' name='chkLocID[]' id="chkLoc" value='<?php echo $Employee->emp_number.'_'.$Employee->eval_id ?>' />-->
                                    ...
                                    <td class="">
                                        <a href="#" onclick="update('<?php echo $Employee->EmployeeSubordinate->employeeId; ?>','<?php echo $Employee->PerformanceEvaluation->eval_year;?>','<?php echo $encrypt->encrypt($Employee->emp_number.'_'.$Employee->eval_id); ?>')"><?php echo $Employee->EmployeeSubordinate->employeeId; ?></a>
<!--                                        <a href="<?php echo url_for('performance/UpdateSDOEvaluation?id=' . $encrypt->encrypt($Employee->emp_number.'_'.$Employee->eval_id)) ?>"><?php echo $Employee->EmployeeSubordinate->employeeId; ?></a>-->
                                    ...
                                    <td class="">
                                     <a href="#" onclick="update('<?php echo $Employee->EmployeeSubordinate->employeeId; ?>','<?php echo $Employee->PerformanceEvaluation->eval_year;?>','<?php echo $encrypt->encrypt($Employee->emp_number.'_'.$Employee->eval_id); ?>')">   
...
//}
        ...
        <script type="text/javascript">
           function update(empno,year,id){
               //alert(empno+"kkk"+year);
               $.ajax({
                type: "POST",
                async:false,
                url: "<?php echo url_for('performance/EmployeeProjectWebService') ?>",
               ...
               window.open("<?php echo url_for('performance/UpdateSDOEvaluation?id='); ?>"+id,"_self","resizable,location,menubar,toolbar,scrollbars,status");
           }
